...
class BottleneckTransform(nn.Sequential):
    ...
    def __init__(
        ...
        w_b = int(round(width_out * bottleneck_multiplier))  # bottleneck_multiplier > 1 for inverted bottleneck
        g = w_b // group_width
        assert len(norm_layer) == 3
        assert len(activation_layer) == 3
        assert g > 0, f"Group convolution groups {g} should be greater than 0."
        assert w_b % g == 0, f"Convolution input channels {w_b} is not divisible by {g} groups."

        layers: OrderedDict[str, nn.Module] = OrderedDict()
        layers["a"] = ConvBlock(
            width_in, w_b, kernel_size=1, stride=1, norm_layer=norm_layer[0], activation_layer=activation_layer[0], inplace=True if activation_layer[0] in INPLACE_ACTIVATIONS else None
        )

        layers["b"] = ConvBlock(
            w_b, w_b, kernel, stride=stride, groups=g, dilation=dilation, norm_layer=norm_layer[1], activation_layer=activation_layer[1], inplace=True if activation_layer[1] in INPLACE_ACTIVATIONS else None
        )
        ...
        if ConvBlock == Conv2dNormActivation:
            layers["c"] = ConvBlock(
                w_b, width_out, kernel_size=1, stride=1, norm_layer=norm_layer[2], activation_layer=None
            )
        else:
            layers["c"] = ConvBlock(
                w_b, width_out, kernel_size=1, stride=1, norm_layer=norm_layer[2], activation_layer=activation_layer[2], inplace=True if activation_layer[2] in INPLACE_ACTIVATIONS else None
            )
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(
        ...
        if should_proj:
            if ConvBlock == Conv2dNormActivation:
                self.proj = ConvBlock(
                    width_in, width_out, kernel_size=1, stride=stride, norm_layer=downsample_norm, activation_layer=None
                )
            elif ConvBlock == NormActivationConv2d:
                self.proj = ConvBlock(
                    width_in, width_out, kernel_size=1, stride=stride, norm_layer=None, activation_layer=None, bias=False
                )
...
class Stem(nn.Module):
    ...
    def __init__(
        ...
        layers["stem"] = Conv2dNormActivation(
            width_in, width_out, kernel_size=kernel_size, stride=stride, norm_layer=norm_layer, activation_layer=activation_layer
        )

        if not patch_size and downsample_factor // 2 > 1:
            layers["stem_downsample"] = nn.MaxPool2d(kernel_size=3, stride=downsample_factor // 2, padding=1)
...
class ConfigurableModel(nn.Module):
    def __init__(
        ...
        norm_layer: list[Callable[..., nn.Module]],  # norm layer in each block, length 3 for bottleneck
        activation_layer: list[Callable[..., nn.Module]],  # activation layer in each block, length 3 for bottleneck
        group_widths: list[int],  # group conv width in each stage, groups = width_out * bottleneck_multiplier // group_width
        bottleneck_multipliers: list[float],  # bottleneck_multiplier > 1 for inverted bottleneck
        downsample_norm: Callable[..., nn.Module],  # norm layer in downsampling shortcut
        ...
        block_constructor: Callable[..., nn.Module] = BottleneckBlock,  # block type in body stage
        ConvBlock: Callable[..., nn.Module] = Conv2dNormActivation,  # block with different "conv-norm-act" order
        se_ratio: Optional[float] = None,  # squeeze and excitation (SE) ratio
        se_activation: Optional[Callable[..., nn.Module]] = None,  # activation layer in SE block
        ...
        assert sum([i % 8 for i in stage_widths]) == 0, f"Stage width {stage_widths} non-divisible by 8"
        ...
        stages = list()
        for i, (width_out, stride, group_width, bottleneck_multiplier, depth) in enumerate(zip(stage_widths, strides, group_widths, bottleneck_multipliers, depths)):
            stages.append((
                f"stage{i + 1}",
                Stage(
                    current_width,
